...
/* eslint-disable */
import { ASTNode, formatError, GraphQLError, GraphQLFormattedError, printError, Source, SourceLocation } from 'graphql';
...
export function toApolloError(
  ...
  const error_ = error;
  if (error_.extensions) {
    error_.extensions.code = code;
  } else {
    error_.extensions = { code };
  }
  return error_ as Error & { extensions: Record<string, any> };
}
...
export function fromGraphQLError(error: GraphQLError, options?: ErrorOptions) {
  ...
  for (const [key, value] of Object.entries(error)) {
    if (key === 'extensions') {
      continue; // extensions are handled bellow
    }
    copy[key] = value;
  }
  ...
  return enrichedErrors.map((error) => {
    try {
      ...
      if (debug) {
        ...
        return enrichError(error_ as Partial<GraphQLError>, debug);
      }
      // obscure error
      const newError = fromGraphQLError(new GraphQLError('Internal server error'));
